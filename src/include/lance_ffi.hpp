...
int32_t lance_dataset_update_config(void *dataset, const char *key,
                                    const char *value);
int32_t lance_dataset_update_config_entries(void *dataset,
...
// Native lease protocol.  The lock record is stored in the dataset's object
// store, so independent Vane processes and Ray workers that use the same
// storage credentials observe the same lease.  The returned handle is opaque
// and must be released exactly once unless the caller intentionally retains it
// for outcome-unknown reconciliation.
enum LanceLeaseKind : uint8_t {
  LANCE_LEASE_SNAPSHOT = 1,
  LANCE_LEASE_MUTATION = 2,
  LANCE_LEASE_VACUUM = 3,
};
void *lance_acquire_lease_with_storage_options(const char *path, uint8_t kind,
                                               const char *operation_id,
                                               const char **option_keys,
                                               const char **option_values,
                                               size_t options_len,
                                               void *session, uint64_t wait_ms);
void *lance_acquire_dataset_lease(void *dataset, uint8_t kind,
                                  const char *operation_id, uint64_t wait_ms);
int32_t lance_release_lease(void *lease);
void lance_close_lease(void *lease);
const char *lance_lease_token(void *lease);
